...
//                                  the modes, spelled rather than typed
...
//
// Five modes, because a title field is not a markdown document and neither is a
// shell script or a namespace of Clojure. `install(view, commands)` is the
// markdown layout, as it always was — every caller that passes no mode is
// untouched by modes existing — and `{mode: 'text'}`, `{mode: 'shell'}`,
// `{mode: 'clojure'}` and `{mode: 'input'}` are the other four. Pair the last
// with `singleLine(cm)`: the layout says what the keys do, singleLine says what
// the document may be, and a field wants both.
//
...
// the part worth testing — and they are where structured LISP editing came
// from: sexp.js was written for the four motions inside a ```clojure fence, and
// 'clojure' mode is those same four with the whole file for bounds.

export {install, bindings,
        markdownBindings, textBindings, shellBindings, clojureBindings,
        inputBindings, documentBindings,
        chord, motion, selectTo, sexpAware, inShellFence, swallow,
        MARKDOWN, TEXT, SHELL, CLOJURE, INPUT, DOCUMENT} from './bindings.js';
